...
(*******************************************************************************
      ...
      Unit containing records for handling asynchronous events to transform a
      synchronous function into an asynchronous function.

      The  Async.Support  unit  provides  definitions for the
      TAsyncParams<T>  and  TAsynStreamParams<T>  records, which  are  used
      to  handle  the lifecycle  of asynchronous operations, including
      starting, progressing, succeeding, and handling errors.
      These records enable non-blocking operations pour une fonction synchrone
      and can be reused across multiple modules.

      This  unit  depends  on Async.Params  for parameter  management.
      The IUseParams<T>  and  TUseParamsFactory<T>  interfaces  and  classes
      from  Async.Params  are  utilized  to  create and manage the
...
uses
  System.SysUtils, system.Classes, System.Threading, Async.Params;
