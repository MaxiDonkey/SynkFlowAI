...
(*******************************************************************************
      ...
      The Async.Params  unit  provides  a set of tools for creating
      ...
      These abstractions allow a flexible and reusable way to handle parameters 
      in different modules and contexts, in particular to transform a synchronous 
      function into an asynchronous function.

      Note  that  This  unit  is  designed   to  work   seamlessly  with  the
      Async.Support unit,  which  relies  on  IUseParams<T> and
      TUseParamsFactory<T>  to  manage   parameters   for  asynchronous  
      requests.
